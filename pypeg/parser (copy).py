...
class Instruction(object):
    def __init__(self,name,label,goto=None,charlist=None, idx=None, size=None,character=None):
        self.name=name
        self.label=label
        self.goto=goto
...
def line_to_instruction(line):
    ...
    if "->" in line:#assuming format of "stuff -> int"
        ...
        line = gotosplit[0]
    if "[" in line and "]" in line:#assuming format similar to "labelname [(stuff)(stuff)]"
        cut1=line.find("[")
        cut2=line.find("]")+1
    ...
    while name[-1]==" ":
        name=name[:-1]
    while name[0]==" ":
        name=name[1:]#didnt use strip() method because some bytecodes have spaces in the middle of them
...
if __name__=="__main__":
    ...
    instructionlist=parse(bytecode)
