...
>>>HIER MEHR INFOS: welcher PC, welche hardware(prozessor, RAM), welches OS, welche versionen von lua, grep<<<
Intel(R) Core(TM) i5-2430M CPU @ 2.40GHz
cache size      : 3072 KB
ubuntu 14.04 lts
Lua 5.2.3
grep (GNU grep) 2.16
>>>NOCH MEHR INFOS: was machen die benchmarks (shell aufruf des programms, seperater prozess, 100 mal)<<<
...
optimizations do not outweigh the time required to generate these
optimizations. As a result, all of our attempts perform significantly slower
than LPeg.
...
This time we are looking at a different pattern, one that parses JSON used on a
